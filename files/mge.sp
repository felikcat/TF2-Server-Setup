...
Action Timer_Tele(Handle timer, int userid)
{
    ...
    if (TF2_GetPlayerClass(client) == TFClass_Engineer)
    {
        ...
        {
            int itemdef = GetEntProp(melee, Prop_Send, "m_iItemDefinitionIndex");
            // eureka effect itemdef
            if (itemdef == 589)
            {
                // just in case.
                RemoveEntity(melee);
                PrintToChat(client, "[MGE] You cannot use The Eureka Effect. Automatically removing you from the queue.");
                // run elo calc so clients can't be cheeky if they're losing
                RemoveFromQueue(client, true);
            }
        }
